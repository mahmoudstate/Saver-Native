// Offers to restore a previous iCloud auto-backup on a genuinely empty
// install (fresh reinstall on the same device, or a brand-new device signed
// into the same Apple ID), the same encrypted file/key useICloudAutoBackup
// writes, read back with no password prompt.
//
...
import { icloudAvailable, readICloudBackup } from "../lib/icloudBackup.js";
import { useT } from "../lib/i18n.js";

export function useICloudRestoreCheck(store) {
  const [checking, setChecking] = useState(true);
  const tr = useT();
  const isEmpty = (store.txns?.length ?? 0) === 0 && (store.banks?.length ?? 0) === 0;
  const dismissed = store.iCloudRestoreDismissed;

  useEffect(() => {
    if (Capacitor.getPlatform() !== "ios" || !isEmpty || dismissed) {
      setChecking(false);
      return;
    }
    let cancelled = false;
    ...
    (async () => {
      try {
        if (await icloudAvailable()) {
          const enc = await readICloudBackup();
          // No key yet is most likely iCloud Keychain still syncing it in,
          // so skip silently this run rather than generating a fresh (wrong) key.
          const key = enc ? await peekAutoBackupKey() : null;
          if (enc && key) {
            const payload = await decryptBackup(enc, key);
            if (!cancelled) {
              store.setConfirm({
                title: tr("privacy.foundICloudBackupTitle"),
                message: tr("privacy.foundICloudBackupMsg"),
                ...
                onCancel: () => store.set("iCloudRestoreDismissed", true),
              });
            }
          }
        }
      } catch (e) {
        console.warn("[icloud-restore] check failed:", e);
      } finally {

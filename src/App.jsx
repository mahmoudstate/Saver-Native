...
export default function App() {
  ...
  else if (view?.type === "budgets") viewScreen = <Budgets store={store} initialSeg={budgetsSeg} onSegChange={setBudgetsSeg} back={back} onAdd={(seg) => push({ type: "editBudget", budget: null, kind: seg === "projects" ? "project" : "monthly" })} onOpenBudget={(b) => push({ type: "budget", budgetId: b.id, viewMonth: b._anchor })} onOpenProject={(p) => push({ type: "project", projectId: p.id })} />;
  else if (view?.type === "editBudget") viewScreen = <BudgetEditor store={store} budget={view.budget} initialKind={view.kind} onClose={back} />;
  else if (view?.type === "budget") viewScreen = <BudgetDetail store={store} budgetId={view.budgetId} viewMonth={view.viewMonth} back={back} onEdit={(b) => push({ type: "editBudget", budget: b })} onEditTxn={(t) => push({ type: "edit", txn: t })} />;
